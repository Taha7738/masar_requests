...
frappe.ui.form.on("Leave Application", {
    ...
    setup(frm) {
        ...
        //     The query returns employees from the applicant's department only.
    ...
    from_date(frm) {
        ...
        if (masar_requests_is_any_partial(frm) && frm.doc.from_date) {
            masar_requests_set_value_if_changed(
                frm,
...
function masar_requests_partial_leave_setup(frm) {
    ...
    masar_requests_toggle_if_exists(
        frm,
...
function masar_requests_validate_partial_leave_client(frm) {
    ...
    if (selected > 1) {
        frappe.throw(
            __(
                ...
                "Half Day, Quarter Day, or Hourly Leave."
            )
        );
    }
    ...
    if (selected) {
        ...
        frm.doc.from_time = masar_requests_seconds_to_time(
            masar_requests_display_time_to_seconds(
        ...
        if (cint(frm.doc.is_hourly)) {
            if (!frm.doc.custom_partial_to_time_ar) {
                frappe.throw(
                    __("End Time is required for Hourly Leave.")
                );
            }

            const interval =
                masar_requests_get_live_hourly_interval(frm);
            ...
            if (interval) {
                ...
                frm.doc.to_time =
                    masar_requests_seconds_to_time(
                        masar_requests_display_time_to_seconds(
...
function masar_requests_update_time_fields_live(frm) {
    ...
    if (cint(frm.doc.half_day)) {
        if (
            frm.doc.custom_shift_hours &&
            flt(frm.doc.custom_shift_hours) > 0
        ) {
            const leave_hours =
                flt(frm.doc.custom_shift_hours / 2, 4);
            ...
            masar_requests_set_display_text_direct(
                frm,
                `${__("From")} ` +
                `${masar_requests_seconds_to_display_time(start_seconds)} ` +
                `${__("to")} ` +
                `${masar_requests_seconds_to_display_time(end_seconds)}`
            );
    ...
    if (cint(frm.doc.quarter_day)) {
        if (
            ...
            const leave_hours =
                flt(frm.doc.custom_shift_hours / 4, 4);
    ...
    if (cint(frm.doc.is_hourly)) {
        ...
        masar_requests_set_display_text_direct(
            frm,
            `${__("From")} ` +
            `${masar_requests_seconds_to_display_time(interval.start)} ` +
            `${__("to")} ` +
            `${masar_requests_seconds_to_display_time(interval.end)}`
        );
        ...
        if (
            ...
            masar_requests_set_calc_field_live(
                ...
                flt(
                    interval.hours /
                    flt(frm.doc.custom_shift_hours),
                    4
                )
            );
        }
...
function masar_requests_set_calc_field_live(
    frm,
    fieldname,
    value
) {
...
async function masar_requests_partial_leave_preview(frm) {
    ...
    let shift_start =
        masar_requests_time_to_seconds(shift.start_time);

    let shift_end =
        masar_requests_time_to_seconds(shift.end_time);
    ...
    const shift_hours =
        flt((shift_end - shift_start) / 3600, 4);
    ...
    if (cint(frm.doc.half_day)) {
        ...
        if (
            !masar_requests_interval_inside_shift(
                start_seconds,
                end_seconds,
                shift_start,
                shift_end
            ...
            frappe.msgprint({
                ...
                message:
                    __(
                        "Half-day leave time exceeds the shift time " +
                        "for today. The shift ends at "
                    ) +
                    masar_requests_seconds_to_display_time(
                        shift_end
                    )
            });
        ...
        await masar_requests_set_value_if_changed(
            frm,
            "from_time",
            masar_requests_seconds_to_time(start_seconds)
        );

        await masar_requests_set_value_if_changed(
            frm,
            "to_time",
            masar_requests_seconds_to_time(end_seconds)
        );
    ...
    if (cint(frm.doc.quarter_day)) {
        ...
        if (
            ...
            frappe.msgprint({
                ...
                message:
                    __(
                        "Quarter-day leave time exceeds the shift " +
                        "time for today. The shift ends at "
                    ) +
                    masar_requests_seconds_to_display_time(
                        shift_end
                    )
    ...
    if (cint(frm.doc.is_hourly)) {
        ...
        const interval =
            masar_requests_get_hourly_interval_inside_shift(
                frm.doc.custom_partial_from_time_ar,
                frm.doc.custom_partial_to_time_ar,
                shift_start,
                shift_end
        ...
        const leave_days =
            flt(interval.hours / shift_hours, 4);
...
// AR: جلب رصيد الإجازة الدقيق من الخادم وعرضه للمستخدم المخوّل فقط.
//     عندما يعيد الخادم null لمستخدم HR User أثناء عرض طلب موظف آخر،
//     تُمسح قيم الرصيد من الواجهة دون إظهار خطأ أو اعتبار الرصيد صفرًا.
//
// EN: Fetch and display the precise server-side leave balance for
//     authorized users only. When the server returns null for an HR User
//     viewing another employee's request, clear the balance fields without
//     raising an error or treating the balance as zero.
async function masar_requests_update_precise_leave_balance(frm) {
    if (!frm || !frm.doc) return;

    // AR: لا يتم طلب الرصيد قبل تحديد الموظف ونوع الإجازة.
    // EN: Do not request a balance until Employee and Leave Type are selected.
    ...
    // AR: استبعاد الطلب الحالي من الحساب عند تعديل مستند محفوظ.
    // EN: Exclude the current request from calculation when editing a saved document.
    ...
    try {
        ...
        // AR: null يعني أن الرصيد مخفي لهذا المستخدم،
        //     وليس أن الرصيد يساوي صفرًا.
        // EN: null means the balance is hidden from this user,
        //     not that it equals zero.
        if (
            !r ||
            r.message === null ||
            r.message === undefined
        ...
        // AR: لا نخفي أخطاء الخادم الحقيقية؛ نمسح القيم ثم نعيد الخطأ للتشخيص.
        // EN: Do not swallow genuine server errors; clear values and rethrow
        //     for diagnostics.
        masar_requests_set_calc_field_live(
            frm,
            "custom_actual_leave_balance",
...
function masar_requests_update_balance_after_request_local(frm) {
    ...
    // AR:
    // إذا كانت قيمة الرصيد مخفية أو فارغة، فلا نحسب رصيدًا متبقيًا
    // حتى لا يتحول الحقل المخفي إلى قيمة سالبة مبنية على الصفر.
    //
    // EN:
    // If the actual balance is hidden or empty, do not calculate a projected
    // balance so the hidden value is not treated as zero.
    if (
        frm.doc.custom_actual_leave_balance === "" ||
        frm.doc.custom_actual_leave_balance === null ||
        frm.doc.custom_actual_leave_balance === undefined
    ) {
        frm.doc.custom_balance_after_this_request = "";
        frm.refresh_field("custom_balance_after_this_request");
        return;
    }

    const actual_balance =
        flt(frm.doc.custom_actual_leave_balance, 4);

    const request_days =
        flt(frm.doc.total_leave_days || 0, 4);

    const after_request =
        flt(actual_balance - request_days, 4);
...
function masar_requests_schedule_precise_leave_balance(frm) {
    ...
    frm._masar_requests_balance_timer =
        setTimeout(async () => {
            await masar_requests_update_precise_leave_balance(frm);
            masar_requests_hide_leave_application_dashboard(frm);
        }, 300);

    frm._masar_requests_dashboard_timer =
        setTimeout(() => {
            masar_requests_hide_leave_application_dashboard(frm);
        }, 900);
}
...
async function masar_requests_get_employee_shift(employee, date) {
    const assignments = await frappe.db.get_list(
        "Shift Assignment",
        {
            filters: [
                ["employee", "=", employee],
    ...
    // AR: جلب مستند الوردية كاملًا لقراءة جدول أوقات أيام الأسبوع.
    // EN: Fetch the full Shift Type document to access weekday-specific times.
    const shift_doc =
    ...
    // AR: استخدام اسم اليوم بالإنجليزية لمطابقة القيمة المخزنة في الجدول.
    // EN: Use the English day name to match the stored child-table value.
    const day_name =
        moment(date).locale("en").format("dddd");

    // AR: استخدام وقت اليوم المخصص إن وجد، وإلا استخدام وقت الوردية الأساسي.
    // EN: Use weekday-specific times when available, otherwise use defaults.
    if (
        shift_doc.custom_shift_times &&
        shift_doc.custom_shift_times.length > 0
    ) {
        const custom_day =
            shift_doc.custom_shift_times.find(
                (row) => row.day_of_week === day_name
            );
...
function masar_requests_calculate_leave_hours_inside_shift(
    from_value,
    to_value,
    shift_start,
    shift_end
...
function masar_requests_get_hourly_interval_inside_shift(
    from_value,
    to_value,
    shift_start,
    shift_end
    ...
    let leave_start =
        masar_requests_display_time_to_seconds(from_value);

    let leave_end =
        masar_requests_display_time_to_seconds(to_value);

    if (leave_end <= leave_start) {
        const end_plus_12 = leave_end + HALF_DAY;

        if (
            end_plus_12 > leave_start &&
            end_plus_12 <= shift_end
        ) {
    ...
    if (
        ...
        frappe.msgprint({
            ...
            message:
                __(
                    "Leave time is outside shift hours. " +
                    "Today's shift: from "
                ) +
                masar_requests_seconds_to_display_time(
                    shift_start
...
function masar_requests_interval_inside_shift(
    start,
    end,
    shift_start,
    shift_end
...
function masar_requests_inject_leave_application_styles() {
    // AR: تم تعطيل التنسيق المخصص لاستخدام واجهة ERPNext الافتراضية.
    // EN: Custom styling is disabled to use the default ERPNext interface.
    return;
}
...
function masar_requests_get_form_wrapper(frm) {
    ...
    if (
        wrapper[0] instanceof HTMLElement
    ) {
        return wrapper[0];
    }
...
function masar_requests_decorate_leave_application_form(frm) {
    if (
        !frm ||
        frm.doctype !== "Leave Application"
    ) {
        return;
    }

    const wrapper =
        masar_requests_get_form_wrapper(frm);

    if (!wrapper) return;

    wrapper.classList.add(
        "masar_requests-leave-form"
    );
    ...
    const hide_dashboard = () => {
        ...
        dashboards.forEach((dashboard) => {
            ...
            dashboard.setAttribute(
                "data-masar_requests-hidden-dashboard",
                "1"
            );
        });
    };
...
async function masar_requests_set_value_if_changed(
    frm,
    fieldname,
    value
) {
...
function masar_requests_set_if_exists(
    frm,
    fieldname,
    value
) {
...
function masar_requests_toggle_if_exists(
    frm,
    fieldname,
    show
) {
...
// AR: إتاحة بعض الدوال في Console المتصفح لأغراض الفحص.
// EN: Expose selected functions to the browser console for diagnostics.
window.masar_requests_schedule_precise_leave_balance =
    masar_requests_schedule_precise_leave_balance;

window.masar_requests_update_precise_leave_balance =
    masar_requests_update_precise_leave_balance;

window.masar_requests_update_balance_after_request_local =
    masar_requests_update_balance_after_request_local;

window.masar_requests_hide_leave_application_dashboard =
    masar_requests_hide_leave_application_dashboard;

window.masar_requests_inject_leave_application_styles =
    masar_requests_inject_leave_application_styles;

window.masar_requests_decorate_leave_application_form =
    masar_requests_decorate_leave_application_form;

window.masar_requests_get_form_wrapper =
    masar_requests_get_form_wrapper;

window.masar_requests_leave_debug = function () {
    const wrapper =
        masar_requests_get_form_wrapper(cur_frm);

    return {
        style_loaded:
            Boolean(
                document.getElementById(
                    "masar_requests-leave-application-style"
                )
            ),
        ...
        hide_fn:
            typeof masar_requests_hide_leave_application_dashboard,

        actual:
            cur_frm && cur_frm.doc
                ? cur_frm.doc.custom_actual_leave_balance
                : null,

        after:
            cur_frm && cur_frm.doc
                ? cur_frm.doc.custom_balance_after_this_request
                : null,

        total_days:
            cur_frm && cur_frm.doc
                ? cur_frm.doc.total_leave_days
                : null,
    };
};
...
// AR: التحقق من أن المستخدم HR User للعرض فقط وليس مدير موارد بشرية.
// EN: Check whether the user is a read-only HR User, not an HR Manager.
...
// AR: تحديد ما إذا كان الطلب شخصيًا للمستخدم الحالي.
// EN: Determine whether the request belongs to the current user.
function masar_requests_leave_is_personal_request(frm) {
    // AR:
    // الطلب الجديد يعد شخصيًا، وكذلك المستند الذي أنشأه المستخدم نفسه.
    //
    // EN:
    // A new request is personal, as is a document owned by the current user.
    return Boolean(
        frm.is_new() ||
...
// AR: قفل طلبات الموظفين الآخرين لمستخدم HR User مع إبقاء طلبه الشخصي متاحًا.
// EN: Lock other employees' requests for HR User while preserving personal requests.
function masar_requests_leave_apply_hr_user_read_only(frm) {
    // AR:
    // لا نطبق القفل إذا لم يكن المستخدم HR User للعرض فقط،
    // أو إذا كان الطلب شخصيًا للمستخدم الحالي.
    //
    // EN:
    // Do not apply the lock unless the user is a read-only HR User
    // viewing another employee's request.
    if (
    ...
    Object.keys(
        ...
        const fieldtype =
            frm.fields_dict[fieldname]?.df?.fieldtype;
