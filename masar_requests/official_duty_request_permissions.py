"""
AR: منطق الصلاحيات والتحكم في الوصول ضمن الوحدة `official_duty_request_permissions`.
EN: Permission and access-control logic for the `official_duty_request_permissions` module.
"""

# ============================================================================
# AR: صلاحيات وتحقق وإشعارات مستند المهمة الرسمية المستقل
...
class _RichTextExtractor(HTMLParser):
    """
    AR: فئة `_RichTextExtractor` لتنظيم منطق الرسمية المهمة الطلب الصلاحيات.
    EN: Class `_RichTextExtractor` that organizes official duty request permissions logic.
    """

    def __init__(self):
        """
        AR: تنفيذ `init` ضمن وحدة `official_duty_request_permissions`.
        EN: Execute init within the `official_duty_request_permissions` module.
        """
    ...
    def handle_data(self, data):
        """
        AR: تنفيذ `handle` `data` ضمن وحدة `official_duty_request_permissions`.
        EN: Execute handle data within the `official_duty_request_permissions` module.
        """
...
def _rich_text_has_content(value):
    """
    AR: تنفيذ `rich` `text` التحقق من وجود `content` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute rich text has content within the `official_duty_request_permissions` module.
    """
    if not value:
        return False
...
def _roles(user=None):
    """
    AR: تنفيذ `roles` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute roles within the `official_duty_request_permissions` module.
    """
...
def is_system_administrator(user=None):
    """
    AR: تنفيذ التحقق من كون `system` `administrator` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is system administrator within the `official_duty_request_permissions` module.
    """
...
def is_hr_manager_or_admin(user=None):
    """
    AR: تنفيذ التحقق من كون `hr` المدير `or` `admin` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is hr manager or admin within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
...
def is_hr_read_only_user(user=None):
    """
    AR: تنفيذ التحقق من كون `hr` القراءة `only` المستخدم ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is hr read only user within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
    roles = _roles(user)
    return (
...
def _employee_user(employee):
    """
    AR: تنفيذ الموظف المستخدم ضمن وحدة `official_duty_request_permissions`.
    EN: Execute employee user within the `official_duty_request_permissions` module.
    """
    if not employee:
        return None
...
def _applicant_user(doc):
    """
    AR: تنفيذ `applicant` المستخدم ضمن وحدة `official_duty_request_permissions`.
    EN: Execute applicant user within the `official_duty_request_permissions` module.
    """
    return (
...
def _is_applicant(doc, user=None):
    """
    AR: تنفيذ التحقق من كون `applicant` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is applicant within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
...
def _is_substitute(doc, user=None):
    """
    AR: تنفيذ التحقق من كون `substitute` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is substitute within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
...
def _is_direct_manager(doc, user=None):
    """
    AR: تنفيذ التحقق من كون `direct` المدير ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is direct manager within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
...
def _is_related_user(doc, user):
    """
    AR: تنفيذ التحقق من كون `related` المستخدم ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is related user within the `official_duty_request_permissions` module.
    """
    return bool(
        doc.get("owner") == user
...
def official_duty_request_query(user=None):
    """
    AR: تنفيذ الرسمية المهمة الطلب `query` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute official duty request query within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
    ...
    return f"""(
        `tabOfficial Duty Request`.`owner` = {escaped_user}
        OR `tabOfficial Duty Request`.`custom_applicant_user` = {escaped_user}
        OR `tabOfficial Duty Request`.`custom_substitute_user` = {escaped_user}
        OR `tabOfficial Duty Request`.`custom_direct_manager_user` = {escaped_user}
        OR EXISTS (
            SELECT 1
              FROM `tabEmployee` applicant_employee
             WHERE applicant_employee.`name` = `tabOfficial Duty Request`.`employee`
...
def official_duty_request_has_permission(
    ...
    """
    AR: تنفيذ الرسمية المهمة الطلب التحقق من وجود صلاحية ضمن وحدة `official_duty_request_permissions`.
    EN: Execute official duty request has permission within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
...
def _current_user_employee(user=None, required=True):
    """
    AR: تنفيذ الحالي المستخدم الموظف ضمن وحدة `official_duty_request_permissions`.
    EN: Execute current user employee within the `official_duty_request_permissions` module.
    """
    user = user or frappe.session.user
...
def get_current_user_employee():
    """
    AR: تنفيذ استرجاع الحالي المستخدم الموظف ضمن وحدة `official_duty_request_permissions`.
    EN: Execute get current user employee within the `official_duty_request_permissions` module.
    """
    if frappe.session.user == "Guest":
        frappe.throw(_("Please sign in before creating an Official Duty Request."))
...
def set_request_employee_from_session(doc):
    """
    AR: تنفيذ تعيين الطلب الموظف `from` `session` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute set request employee from session within the `official_duty_request_permissions` module.
    """
    ...
    if doc.is_new():
        if is_system_administrator(user):
            ...
            if doc.get("employee") and doc.get("employee") != employee.name:
                frappe.throw(
                    _(
                        "You can create an Official Duty Request only for your own Employee record."
                    ),
...
def _is_empty_value(value):
    """
    AR: تنفيذ التحقق من كون `empty` `value` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is empty value within the `official_duty_request_permissions` module.
    """
...
def _normalize_time_for_compare(value):
    """
    AR: تنفيذ توحيد الوقت `for` `compare` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute normalize time for compare within the `official_duty_request_permissions` module.
    """
...
def _normalize_field_value(field, value):
    """
    AR: تنفيذ توحيد الحقل `value` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute normalize field value within the `official_duty_request_permissions` module.
    """
    if _is_empty_value(value):
        return None
...
def _same_field_value(doc, previous, fieldname):
    """
    AR: تنفيذ `same` الحقل `value` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute same field value within the `official_duty_request_permissions` module.
    """
...
def _get_previous_doc(doc):
    """
    AR: تنفيذ استرجاع `previous` المستند ضمن وحدة `official_duty_request_permissions`.
    EN: Execute get previous doc within the `official_duty_request_permissions` module.
    """
...
def _get_previous_workflow_state(doc):
    """
    AR: تنفيذ استرجاع `previous` سير العمل الحالة ضمن وحدة `official_duty_request_permissions`.
    EN: Execute get previous workflow state within the `official_duty_request_permissions` module.
    """
...
def _can_reselect_substitute(doc, previous):
    """
    AR: تنفيذ التحقق من إمكانية `reselect` `substitute` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute can reselect substitute within the `official_duty_request_permissions` module.
    """
    return bool(
        previous
...
def _can_complete_legacy_report(doc, previous, user=None):
    """
    AR: تنفيذ التحقق من إمكانية `complete` القديم التقرير ضمن وحدة `official_duty_request_permissions`.
    EN: Execute can complete legacy report within the `official_duty_request_permissions` module.
...
def validate_locked_request_fields(doc):
    """
    AR: تنفيذ التحقق من صحة `locked` الطلب الحقول ضمن وحدة `official_duty_request_permissions`.
    EN: Execute validate locked request fields within the `official_duty_request_permissions` module.
    """
    previous = _get_previous_doc(doc)
    if not previous:
        return
...
@frappe.whitelist()
def get_official_duty_leave_conflicts(employee, from_date, to_date):
    """
    AR: تنفيذ استرجاع الرسمية المهمة الإجازة `conflicts` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute get official duty leave conflicts within the `official_duty_request_permissions` module.

    DETAILS / التفاصيل:
    Return submitted and approved Leave Applications overlapping the
        requested Official Duty period.
    """
...
def _validate_no_approved_leave_overlap(doc):
    """
    AR: تنفيذ التحقق من صحة `no` `approved` الإجازة `overlap` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute validate no approved leave overlap within the `official_duty_request_permissions` module.

    DETAILS / التفاصيل:
    An employee cannot have an approved leave and an Official Duty
        Request covering the same calendar date.
    """
    conflicts = get_official_duty_leave_conflicts(
        doc.get("employee"),
        doc.get("from_date"),
        doc.get("to_date"),
    )
    ...
    for leave in conflicts:
        conflict_lines.append(
            "<li><b>{name}</b> — {leave_type} "
            "({from_date} إلى {to_date})</li>".format(
                name=frappe.utils.escape_html(leave.name),
                leave_type=frappe.utils.escape_html(
                    leave.leave_type or ""
                ),
                from_date=frappe.format_value(
                    leave.from_date,
                    {"fieldtype": "Date"},
                ),
                to_date=frappe.format_value(
                    leave.to_date,
                    {"fieldtype": "Date"},
                ),
            )
    ...
    frappe.throw(
        """
        لا يمكن حفظ طلب المهمة الرسمية لأن الموظف لديه
        إجازة معتمدة متداخلة مع تاريخ المهمة.
        <br><br>
        <ul>{conflicts}</ul>
        <br>
        يجب إلغاء الإجازة أو اختيار تاريخ آخر قبل إنشاء المهمة الرسمية.
        """.format(
            conflicts="".join(conflict_lines)
        ),
        title="تعارض مع إجازة معتمدة",
    )


def _validate_official_duty_fields(doc):
    """
    AR: تنفيذ التحقق من صحة الرسمية المهمة الحقول ضمن وحدة `official_duty_request_permissions`.
    EN: Execute validate official duty fields within the `official_duty_request_permissions` module.
    """
    _validate_no_approved_leave_overlap(doc)
    """
    AR:
        التحقق من التواريخ ونوع المهمة وحقول الساعات والتقرير. المهمة
        بالساعات تقتصر على يوم واحد، بينما المهمة اليومية تدعم عدة أيام.

    EN:
        Validate dates, duty type, hourly fields, and the achievement report.
        Hourly duty is intentionally limited to one day; full-day duty may span dates.
    """
    doc.reason = "On Duty"
    ...
    if doc.get("duty_type") == "Hourly":
        if getdate(doc.from_date) != getdate(doc.to_date):
            frappe.throw(_("Hourly Official Duty must be submitted for one date only."))
        for fieldname in ("custom_leaving_time", "custom_return_time"):
            if not doc.get(fieldname):
                field = doc.meta.get_field(fieldname)
                label = _(field.label) if field and field.label else fieldname
                frappe.throw(_("Please fill in the required field: {0}").format(label))
        ...
        if leaving_seconds == return_seconds:
            frappe.throw(_("Leaving Time and Return Time cannot be the same."))
    else:
        # AR: منع بقاء أوقات قديمة عند تغيير النوع من Hourly إلى Full Day.
        # EN: Clear stale times when changing from Hourly to another duty type.
        doc.custom_leaving_time = None
        doc.custom_return_time = None
    ...
    _validate_request_overlap(doc)


def _validate_request_overlap(doc):
    """
    AR: تنفيذ التحقق من صحة الطلب `overlap` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute validate request overlap within the `official_duty_request_permissions` module.
    """
    ...
    overlapping = frappe.db.get_value(
        OFFICIAL_DUTY_DOCTYPE,
        {
            ...
            "docstatus": ("<", 2),
            "name": ("!=", doc.name or "New Official Duty Request"),
    ...
    if overlapping:
        frappe.throw(
            _("Official Duty Request {0} overlaps this period.").format(
...
def set_substitute_user(doc):
    """
    AR: تنفيذ تعيين `substitute` المستخدم ضمن وحدة `official_duty_request_permissions`.
    EN: Execute set substitute user within the `official_duty_request_permissions` module.
    """
...
def set_direct_manager_from_reports_to(doc):
    """
    AR: تنفيذ تعيين `direct` المدير `from` `reports` `to` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute set direct manager from reports to within the `official_duty_request_permissions` module.
    """
    if not doc.get("employee"):
        return
...
def _requested_workflow_action():
    """
    AR: تنفيذ `requested` سير العمل `action` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute requested workflow action within the `official_duty_request_permissions` module.
    """
...
def _is_transition(action, expected, previous_state, state, from_states, to_state):
    """
    AR: تنفيذ التحقق من كون `transition` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute is transition within the `official_duty_request_permissions` module.
    """
    if action:
...
def _require_actor(condition, message):
    """
    AR: تنفيذ `require` `actor` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute require actor within the `official_duty_request_permissions` module.
    """
...
def _clear_substitute_selection(doc):
    """
    AR: تنفيذ `clear` `substitute` `selection` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute clear substitute selection within the `official_duty_request_permissions` module.
    """
...
def sync_approval_status_fields(doc):
    """
    AR: تنفيذ مزامنة `approval` الحالة الحقول ضمن وحدة `official_duty_request_permissions`.
    EN: Execute sync approval status fields within the `official_duty_request_permissions` module.
...
def validate_official_duty_request(doc, method=None):
    """
    AR: تنفيذ التحقق من صحة الرسمية المهمة الطلب ضمن وحدة `official_duty_request_permissions`.
    EN: Execute validate official duty request within the `official_duty_request_permissions` module.
    """
    # AR: مسار ترحيل داخلي فقط؛ يمنع إعادة تنفيذ شروط الطلب الجديد على السجلات التاريخية.
    # EN: Internal migration path only; historical rows bypass new-request validation.
    if getattr(doc.flags, "legacy_migration", False):
        doc.reason = "On Duty"
        return
...
def before_submit_official_duty_request(doc, method=None):
    """
    AR: تنفيذ معالجة ما قبل اعتماد الرسمية المهمة الطلب ضمن وحدة `official_duty_request_permissions`.
    EN: Execute before submit official duty request within the `official_duty_request_permissions` module.
    """
    sync_approval_status_fields(doc)
...
def _grant_docshare(doc, user, write=0):
    """
    AR: تنفيذ منح `docshare` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute grant docshare within the `official_duty_request_permissions` module.
    """
...
def sync_official_duty_request_shares(doc, method=None):
    """
    AR: تنفيذ مزامنة الرسمية المهمة الطلب `shares` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute sync official duty request shares within the `official_duty_request_permissions` module.
    """
...
def resync_all_official_duty_request_shares():
    """
    AR: تنفيذ `resync` `all` الرسمية المهمة الطلب `shares` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute resync all official duty request shares within the `official_duty_request_permissions` module.
    """
    names = frappe.get_all(OFFICIAL_DUTY_DOCTYPE, pluck="name")
    for name in names:
        sync_official_duty_request_shares(frappe.get_doc(OFFICIAL_DUTY_DOCTYPE, name))
    frappe.db.commit()
    return len(names)
...
def get_users_with_role_safe(role):
    """
    AR: تنفيذ استرجاع `users` `with` الدور `safe` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute get users with role safe within the `official_duty_request_permissions` module.
    """
...
def _notification_already_exists(doc, target, subject):
    """
    AR: تنفيذ الإشعار `already` `exists` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute notification already exists within the `official_duty_request_permissions` module.
    """
    return bool(
        frappe.db.exists(
            "Notification Log",
...
def _translate_notification_for_user(target, source, *args):
    """
    AR: تنفيذ `translate` الإشعار `for` المستخدم ضمن وحدة `official_duty_request_permissions`.
    EN: Execute translate notification for user within the `official_duty_request_permissions` module.
...
def _create_notification(doc, target, source, *args):
    """
    AR: تنفيذ إنشاء الإشعار ضمن وحدة `official_duty_request_permissions`.
    EN: Execute create notification within the `official_duty_request_permissions` module.
    """
    if (
        not target
...
def _applicant_name(doc):
    """
    AR: تنفيذ `applicant` `name` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute applicant name within the `official_duty_request_permissions` module.
    """
    return (
...
def send_official_duty_workflow_notifications(doc):
    """
    AR: تنفيذ `send` الرسمية المهمة سير العمل `notifications` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute send official duty workflow notifications within the `official_duty_request_permissions` module.
    """
...
def _on_update_official_duty_request_base_v21_7(doc, method=None):
    """
    AR: تنفيذ معالجة حدث تحديث الرسمية المهمة الطلب `base` `v21` `7` ضمن وحدة `official_duty_request_permissions`.
    EN: Execute on update official duty request base v21 7 within the `official_duty_request_permissions` module.
    """
    sync_official_duty_request_shares(doc)
    send_official_duty_workflow_notifications(doc)

# MASAR_MANUAL_REVIEW_NOTIFICATION_V21_7
def on_update_official_duty_request(doc, method=None):
    """
    AR: تنفيذ معالجة حدث تحديث الرسمية المهمة الطلب ضمن وحدة `official_duty_request_permissions`.
    EN: Execute on update official duty request within the `official_duty_request_permissions` module.
    """
    _on_update_official_duty_request_base_v21_7(doc, method)
    from masar_requests.workflow_notification_completion import (
        notify_official_duty_manual_review,
    )
    notify_official_duty_manual_review(doc)


def on_submit_official_duty_request(doc, method=None):
    """
    AR: تنفيذ معالجة حدث اعتماد الرسمية المهمة الطلب ضمن وحدة `official_duty_request_permissions`.
    EN: Execute on submit official duty request within the `official_duty_request_permissions` module.
    """
    sync_official_duty_request_shares(doc)
    send_official_duty_workflow_notifications(doc)

    from masar_requests.official_duty_engine import enqueue_official_duty_reconciliation

    enqueue_official_duty_reconciliation(doc)
